...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int querySlection  = 12;

        if (querySlection == 123) {
            String queryFile = System.getProperty("user.dir") + "\\queries123.txt";
        ...
        if (querySlection == 12) {
            String queryFile = System.getProperty("user.dir") + "\\queries12.txt";
            File file = new File(queryFile);
            file.delete();
            BufferedWriter writer = new BufferedWriter(new FileWriter(queryFile, true));
            for (int i = 0; i < getFirstBlock().size(); i++) {
                ...
                for (int j = 0; j < getSecondBlock().size(); j++) {
                    String string2 = getSecondBlock().get(j);
                        writer.write(string1 + " AND " + string2 + "\n");
                    }
            }
            writer.close();
        }
